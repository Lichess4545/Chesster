...
function formatHelpResponse(bot, config) {
    return Q.fcall(function() {
        var leagueNames = _.map(
            league.getAllLeagues(bot, config),
            'options.name'
        )
...
function formatAGameIsScheduled(bot, target, context) {
    ...
    if (target && target.tz_offset) {
        var targetDate = context.result.date
            .clone()
            .utcOffset(target.tz_offset / 60)
        context['yourDate'] = targetDate.format(friendlyFormat)
        message =
            '{white.name} vs {black.name} in {leagueName} has been scheduled for {realDate}, which is {yourDate} for you.'
    } else {
        message =
            '{white.name} vs {black.name} in {leagueName} has been scheduled for {realDate}.'
    }
...
function formatAGameStarts(bot, target, context) {
    return '{white.name} vs {black.name} in {leagueName} has started: {result.gamelink}'.format(
        context
    )
}
...
function formatAGameIsOver(bot, target, context) {
    return '{white.name} vs {black.name} in {leagueName} is over. The result is {result.result}.'.format(
        context
    )
}
...
function processTellCommand(bot, config, message) {
    return Q.fcall(function() {
        ...
        var isCaptain =
            captainName &&
            _.isEqual(_.toLower(captainName[0]), _.toLower(message.player.name))
    ...
    return Q.fcall(function() {
        ...
        return db.Subscription.findAll({
            ...
            _.each(subscriptions, function(subscription) {
                ...
                response += '\nID {id} -> tell {target} when {event} for {source} in {league}'.format(
                    context
                )
            })
...
function getListeners(bot, leagueName, sources, event) {
    return Q.fcall(function() {
        ...
        var teamNames = _(sources)
            .map(function(n) {
                return _league.getTeamByPlayerName(n)
            })
            .filter(_.isObject)
            .map('name')
            .map(_.toLower)
        ...
        return _league.getTeams().then(function(teams) {
            ...
            _.forEach(teams, function(team) {
                if (!team.slack_channel) {
                    ...
                    processed++
                    ;['a-game-starts', 'a-game-is-over'].forEach(function(
                        event
                    ) {
                        return db.Subscription.findOrCreate({
                            where: {
...
function tellMeWhenHandler(config) {
    return function(bot, message) {
        return bot.startPrivateConversation(message.user).then(function(convo) {
            return processTellCommand(bot, config, message)
...
function helpHandler(config) {
    return function(bot, message) {
        return bot.startPrivateConversation(message.user).then(function(convo) {
            return formatHelpResponse(bot, config)
                .then(function(response) {
                    convo.say(response)
                })
                .catch(function(error) {
...
function listHandler(config) {
    return function(bot, message) {
        return bot.startPrivateConversation(message.user).then(function(convo) {
            return processSubscriptionListCommand(bot, config, message)
                .then(function(response) {
                    convo.say(response)
                })
                .catch(function(error) {
...
function subscribeTeams(config) {
    return function(bot, message) {
        return bot.startPrivateConversation(message.user).then(function(convo) {
            return processTeamSubscribeCommand(bot, config, message)
                .then(function(response) {
                    convo.say(response)
                })
                .catch(function(error) {
