...
type HeltourLinkResponse = any

export function welcomeMessage(bot: SlackBot, message: CommandMessage) {
    if (
        _.isEqual(
            message.channel,
        ...
        heltour
            .linkSlack(bot.config.heltour, message.user, '')
            .then(async (result: HeltourLinkResponse) => {
                bot.reply(
                    message,
