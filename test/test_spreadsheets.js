...
describe('Hyperlink', function(){
    it("Test Hyperlink Parsing.", function() {
        ...
        var res = spreadsheets.parse_hyperlink('');
        assert.equal(res["text"], "");
        assert.equal(res["href"], "");

        var res = spreadsheets.parse_hyperlink('=HYPERLINK("http://en.lichess.org/FwYcks48","1-0")');
        assert.equal(res["text"], "1-0");
        assert.equal(res["href"], "http://en.lichess.org/FwYcks48");

        var res = spreadsheets.parse_hyperlink('=HYPERLINK("","1-0")');
        assert.equal(res["text"], "1-0");
        assert.equal(res["href"], "");

        var res = spreadsheets.parse_hyperlink('=HYPERLINK("http://en.lichess.org/FwYcks48","")');
        assert.equal(res["text"], "");
        assert.equal(res["href"], "http://en.lichess.org/FwYcks48");

        var res = spreadsheets.parse_hyperlink('=HYPERLINK("","")');
        assert.equal(res["text"], "");
        assert.equal(res["href"], "");
